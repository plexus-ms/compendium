...
The deploy verb never touches secrets: provisioning owns `secrets.env` (secret values) and `platform.env` (non-secret platform bindings such as the host port — see Ingress), the deploy verb owns `.env` (the image ref), and no two writers share a file. Rotation is changing the vault item and re-running the playbook. Keeping the deploy verb secret-unaware is part of what keeps it a verb, not a system.
