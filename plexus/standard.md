...
`plexus`, the initiative's own dogfooding tenant, is bound by this standard like any other tenant.
...
Concrete tools and services are named in two different strengths:
...
- **Verb** — a stateless, hand-runnable procedure that does one operational thing (deploy, backup, migrate, …), authored in a portable manner.
  One name, two shapes: **ops verbs** are the shared bash scripts `itops` ships (`scripts/deploy.sh`), tenant-neutral and versioned by the `itops` tag; **contract verbs** are the per-app mise tasks the app contract requires (`mise :migrate`), each encoding that app's own incantation behind the standard name.
- **Mount** — the thin, disposable glue binding a verb or role to an event or state source it doesn't own: a workflow wrapper on forge events, a tenant playbook on an inventory, a systemd timer on the clock.
...
- **The paved road** — the composed default path (toolchain, CI, deploy, backup) a new project lands on with near-zero overhead.
  On this road, ties to the reference stack are accepted in exchange for convenience and standardization.
- **Reference stack** — the substitutable concrete choices named without a normative keyword; see § 1.3.
...
Nothing federates at runtime: tenants never talk to each other — no shared identity, no inter-instance protocol; what travels is versioned methodology from one upstream, hub-and-spoke (§ 3.5).
...
With several apps in one repo, deploys are per-app and promotion is repo-wide (the release train — § 7.2); a product that persistently needs its own release cadence is the one reason to give it its own repo within the tenant's org (§ 7.2).

### § 3.5 The supply chain & upstream guarantees

One channel does cross tenant lines: the supply chain.
Whoever controls `plexus-ms` ships code that runs with root on every tenant's hosts (the Ansible roles), inside every tenant's CI (workflows and verbs), and inside every tenant's apps (the packages).
Adopting the standard means trusting its maintainers — a trust channel, not a promise.
...
They are stated in this document because the tenant's risk decision — how unattended to let updates flow (§ 9.1) — rests on them.
...
```mermaid
flowchart TB
  library["plexus-ms/library<br>@plexus-ms/* packages on npm<br>semver · provenance (§ 3.5)"]
  preset["plexus-ms/preset — copier template<br>generates the tenant monorepo (§ 3.4)"]
  itops["plexus-ms/itops<br>verbs · workflow wrappers<br>Ansible roles — one version tag"]
  boundary{{"what crosses: methodology — versioned, pull-based, every pin watched (§ 9)<br>what never crosses: substance — root, secrets, data, hosts, backups (§ 3.1)"}}
  ...
  boundary --> plexus["tenant plexus<br>the initiative as its own tenant"]
```

*Figure 1 (informative) — the federation at a glance: one tenant-neutral upstream, versioned methodology flowing hub-and-spoke to every tenant, substance staying inside each tenant's own trust domain. Each tenant monorepo holds both sides — `apps/` meets `infra/` at the app contract (§ 5); no tenant ↔ tenant edges.*
...
pnpm defines the workspace without a root manifest; the known tool that forces one is changesets, which tenants do not use (§ 7.1) — the exception lives in `plexus-ms/library` and is the [Manual](manual.md)'s business.
...
The shared pipeline (§ 7.4) and the deploy verb (§ 7.3) invoke an app only through these standard names, so the names are load-bearing and the incantations behind them are not.
...
The host side of the binding is not the app's to choose: the host port is assigned by the platform from the tenant's inventory (§ 8.1) and injected at deploy time.
...
The semantics are pinned because the deploy verb's rollback decision rides on this endpoint (§ 7.3), polling it bare over loopback.
Unauthenticated is not the same as private: ingress maps the public domain onto the same single port, so left alone `/healthz` would ride into the open as a free oracle for "is this app's database down" — the platform fences the path at the proxy (§ 8.1), and the empty response is belt and braces.
...
Transient dependency blips are the poller's problem, and handled there (§ 7.3).
...
On the reference stack this is a ~5-line reference to the shared reusable workflow; in a multi-app monorepo the reference is path-scoped per § 7.2.
...
The deploy flow has no down-migration step, and its rollback path (§ 7.3) re-launches the *previous* image against the *already-migrated* schema — which is sound only if additive changes (new tables, nullable columns, backfills) ship first and destructive ones (drops, renames, constraint tightening) ship only in a later release, once no deployed code depends on the old shape.
...
Extending the backup vocabulary means adding one handler upstream — after which every app using that data-service type is covered, with zero bookkeeping (§ 8.3).


## § 7 Releases & deployment
...
The deploy verb needs no change for this: it was per-app all along (§ 7.3), and CI simply fans it out over the changed set.
...
The changed set carries no memory, so CI never *rediscovers* a failed deploy: after the rollback and alert (§ 7.3), the environment branch is ahead of what actually runs, and the next unrelated merge will not close the gap.
Closing it is the operator's move, on paths already paved — re-run the failed CI job (same SHA, same derivation) once the cause is fixed, or invoke the hand-runnable deploy verb directly; never an empty commit.
A red staging deploy directly falsifies `develop`'s prod-readiness, hence the promotion freeze.

Two boundaries of the train:
`infra/` rides no train — it is not an app and the deploy verb never touches it; applying it is a playbook run (§ 8), its own mount, on its own occasions.
And the escape valve is a repo split, not a process patch: if two products in one tenant *persistently* need independent release cadences, move one into its own repo (still inside the tenant's org and access boundary, still on the same contract — § 3.4) — granularity problems are solved by moving a product off the train, never by making promotion partial.

Which host an environment *is*, is inventory, not convention: `main`→prod and `develop`→staging name deploy targets, and the binding — which VM, which `apps[]` record, which domain — lives in the tenant's `infra/` inventory (§ 8.1), where the CI mount reads it.

### § 7.3 Deploy semantics
...
Each app references it in ~5 lines, path-scoped per § 7.2.
...
The forge (reference stack: GitHub, deliberately not self-hosted) is tier-0 for deploys either way: CI is the deploy trigger and GHCR holds the images, so a forge outage means no automated deploys.
...
A reverse proxy per VM (reference stack: Caddy) terminates TLS and maps domains to app ports.
...
Two flows, both resolved when the playbook runs (reference stack: 1Password):
...
Three env files sit in the app directory, and each has exactly one writer: provisioning owns `secrets.env` (secret values) and `platform.env` (non-secret platform bindings such as the host port — § 8.1), the deploy verb owns `.env` (the image ref) — no file has two writers.
...
A failed nightly unit never pings its check, and the missed ping alerts (§ 8.4).
...
```mermaid
flowchart TB
  provisioning["provisioning mount (§ 8.1–8.2)<br>op run --env-file=op.env -- ansible-playbook site.yml<br>reads inventory (apps[]) + vault"]
  deploy["deploy mount (§ 7.3–7.4)<br>push → CI: verbs + image build → deploy verb<br>over ssh: pull · migrate · up · poll /healthz · rollback"]
  subgraph vm["tenant VM (one tenant per VM — § 3.3)"]
    caddy["Caddy (§ 8.1)<br>TLS · domain → host port (both from inventory)<br>refuses public /healthz"]
    subgraph appdir["app directory — compose.yml from git, plus three env files, each with exactly one writer (§ 8.2)"]
    ...
    backup["nightly unit (§ 8.3)<br>dump per plexus.backup label → restic → off-site repo<br>pings its dead-man's-switch check (§ 8.4)"]
  end
...
*Figure 2 (informative) — one tenant VM, assembled: provisioning and the deploy verb write disjoint files, ingress reads the same inventory record that assigns the host port, and backups discover their targets from labels — §§ 8.1–8.4 in one picture.*
...
Which monitor provides the checks and which channel carries the alert are reference-stack choices, currently deferred (see the [Manual](manual.md)'s roadmap); the requirement stands regardless of the tool.
...
A pin no machinery watches is invisible staleness — hence the bot is mandatory while the concrete bot is reference stack: Renovate, extending the shared preset (`plexus-ms/renovate-config`) in one line, is the reference choice this section is written against.

How unattended the merge is, is stratified by blast radius, because one policy does not fit three risk classes: packages run inside an app with immutable, provenance-attested versions; workflow and verb tags run in tenant CI next to its secrets and reference a movable tag (§ 3.5); the Ansible collection runs with root on tenant hosts.
The paved-road default is deliberately not trust-maximal: unattended propagation is granted per risk class, each tenant's bot config records the stratification it chose, and the risk appetite behind that choice is the tenant's own.
...
Across tenants: published, versioned packages (§ 3.5) — the only mechanism that crosses org boundaries, buying decoupled upgrade timing (a consumer pins `^2` and upgrades when *it* chooses) and update-bot propagation (the bot needs a version to detect).
