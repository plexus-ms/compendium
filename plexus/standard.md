...
It specifies the supply chain a tenant consumes (§ 2), the tenant's shape (§ 3), the toolchain (§ 4), the app contract and its profiles (§ 5, § 6), the tenant platform (§ 7), the release and deployment model (§ 8), and the propagation machinery (§ 9); its own scope, conventions, and conformance terms are § 1.
...
This standard binds **tenants implementing Plexus**: their apps (§ 5, § 6), their monorepos (§ 3, § 4, § 8, § 9), and their operations platforms (§ 3, § 7).
Applicability follows from that heading structure — no separate classification is needed: § 5 and the app's declared § 6 profile bind each app; § 3, § 4, § 8, and § 9 bind the tenant monorepo; § 7 binds the tenant platform, which is defined by the monorepo's `infra/`, so the monorepo's marker (§ 1.7) speaks for it.
...
The one exception is § 2: the guarantees the upstream makes to tenants are stated here, because tenants' trust decisions rest on them.
...
- **Reference stack** — the substitutable concrete choices named without a normative keyword; see § 1.4.

### § 1.4 Normative tools & the reference stack
...
Substituting an equivalent for a reference-stack choice is conformant; the normative rule, and where the substitution is owned, is § 1.6.
...
`v0` promises none of that: while the standard is a draft, any revision can change or remove anything, and `plx: v0` in a marker (§ 1.7) means *tracks the draft head* — nothing stronger.
...
A recorded deviation reads as a decision; an unrecorded one reads as drift — the marker (§ 1.7) is where the difference is made visible.
...
Everything a tenant consumes from Plexus arrives over one channel: the supply chain.
...
```mermaid
flowchart TB
  library["plexus-ms/library<br>@plexus-ms/* packages on npm<br>semver · provenance (§ 2)"]
...
On the reference stack this is a ~5-line reference to the shared reusable workflow; in a multi-app monorepo the reference is path-scoped per § 8.2.
...
Every app takes exactly one profile and records it in its `PLEXUS.md` (§ 5, § 1.7); the platform treats every app identically through the same verbs and labels, and never needs to know the profile.
...
A tenant that points an external uptime monitor at it does so as an owned deviation (§ 1.6), knowing what it reveals.
